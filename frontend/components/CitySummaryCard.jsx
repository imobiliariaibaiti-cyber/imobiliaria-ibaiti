"use client";

import { useEffect, useState } from "react";
import { getCitySummary } from "@/lib/api";
import { getCityContentByName } from "@/lib/cities";

export default function CitySummaryCard({ city }) {
  const [data, setData] = useState(null);

  useEffect(() => {
    if (!city) return;

    const predefined = getCityContentByName(city)?.summary;
    if (predefined) {
      setData({ summary: predefined });
      return;
    }

    getCitySummary(city)
      .then(setData)
      .catch(() => setData(null));
  }, [city]);

  if (!data) return null;

  return (
    <div className="rounded-2xl border border-brand-100 bg-brand-50/60 p-4 space-y-1">
      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-brand-700">Sobre {city}</p>
      {data.summary && <p className="text-sm text-slate-700 whitespace-pre-line">{data.summary}</p>}
      {data.rating && <p className="text-sm text-brand-700">Avaliacao geral: {Number(data.rating).toFixed(1)} / 5</p>}
    </div>
  );
}
