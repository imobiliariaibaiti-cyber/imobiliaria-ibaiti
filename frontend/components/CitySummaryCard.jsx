...
export default function CitySummaryCard({ city, manualSummary }) {
  ...
  useEffect(() => {
    ...
    // 1) Se temos um texto padrão, usa imediatamente
    ...
    // 2) Caso contrário, tenta Google Places
    getCitySummary(city)
      .then(setData)
      .catch(() => setData(null));
  }, [city]);
  ...
  return (
    <div className="rounded-2xl border border-brand-100 bg-brand-50/60 p-4 space-y-1">
      ...
      {data.rating && <p className="text-sm text-brand-700">Avaliação geral: {Number(data.rating).toFixed(1)} / 5</p>}
    </div>
  );
}
